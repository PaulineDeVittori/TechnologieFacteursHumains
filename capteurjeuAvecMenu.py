...
def start_menu():
    ...
    pygame.display.set_caption("Jeu de Foot - Menu Principal")
    ...
    # Paramètres pour l'input du score
    input_rect = pygame.Rect(WIDTH // 2 - 50, HEIGHT // 2, 100, 40)
    input_active = False
    score_input = "5"  # Valeur par défaut
    max_input_length = 2  # Maximum 2 chiffres (99 buts max)

    # Paramètres pour le bouton Jouer
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            # Gestion du clic sur le champ de texte
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if button_rect.collidepoint(event.pos):
                    try:
                        win_score = max(1, min(99, int(score_input)))
                        return win_score  # Retourner le score de victoire choisi
                    except ValueError:
                        score_input = "5"  # Valeur par défaut si entrée invalide
            ...
            if event.type == pygame.KEYDOWN:
                if input_active:
                    if event.key == pygame.K_RETURN:
                        ...
                        if event.unicode.isdigit() and len(score_input) < max_input_length:
        ...
        # Effacer l'écran avec un fond vert clair
        screen.fill(LIGHT_GREEN)
        
        # Dessiner des lignes de terrain stylisées
        pygame.draw.rect(screen, GREEN, (50, 50, WIDTH-100, HEIGHT-100), 2)
        pygame.draw.circle(screen, GREEN, (WIDTH//2, HEIGHT//2), 100, 2)
        
        # Animation des ballons en arrière-plan
        for ball in balls:
            ...
            # Dessiner le ballon
            pygame.draw.circle(screen, BLACK, (int(ball['x']), int(ball['y'])), ball['size'])
            # Ajouter des détails au ballon
            pygame.draw.circle(screen, WHITE, (int(ball['x']), int(ball['y'])), ball['size'] - 2, 1)
        
        # Titre du jeu
        title_shadow = font_title.render("JEU DE FOOT", True, BLACK)
        title_text = font_title.render("JEU DE FOOT", True, WHITE)
        screen.blit(title_shadow, (WIDTH // 2 - title_shadow.get_width() // 2 + 3, 103))
        screen.blit(title_text, (WIDTH // 2 - title_text.get_width() // 2, 100))
        
        # Sous-titre avec explication
        subtitle = font_small.render("Contrôle par Capteurs Biométriques", True, WHITE)
        screen.blit(subtitle, (WIDTH // 2 - subtitle.get_width() // 2, 170))
        
        # Texte pour l'input
        input_text = font_medium.render("Nombre de buts pour gagner:", True, WHITE)
        screen.blit(input_text, (WIDTH // 2 - input_text.get_width() // 2, HEIGHT // 2 - 50))
        
        # Dessiner le champ de texte
        pygame.draw.rect(screen, INPUT_ACTIVE_BG if input_active else INPUT_BG, input_rect)
        pygame.draw.rect(screen, BLACK, input_rect, 2)
        
        # Afficher le texte saisi
        input_surface = font_medium.render(score_input, True, BLACK)
        screen.blit(input_surface, (input_rect.x + (input_rect.width - input_surface.get_width()) // 2, 
        ...
        instructions = font_small.render("Utilisez EMG pour tirer et RESP pour ralentir le gardien", True, WHITE)
        screen.blit(instructions, (WIDTH // 2 - instructions.get_width() // 2, HEIGHT - 50))
        
        pygame.display.flip()
        clock.tick(FPS)
    
    # Valeur par défaut si on quitte la boucle sans choisir
    return 5


# ----- Configuration du jeu -----
def game_loop(device, win_score=5):
    pygame.init()
    pygame.mixer.init()  # Initialiser le mixeur pour les sons
    ...
    game_over = False
    # win_score est maintenant passé en paramètre
    ...
    while running:
        ...
        if game_over:
            ...
            menu_text = font_medium.render("'ESC' pour le menu principal", True, WHITE)
    ...
    # Indiquer si on doit retourner au menu
    return "menu" if not running else "quit"
...
if __name__ == "__main__":    
    ...
    while True:
        try:
            # Afficher le menu et récupérer le nombre de buts pour gagner
            win_score = start_menu()
            
            # Lancer le jeu avec le nombre de buts choisi
            result = game_loop(device, win_score)
            
            # Si le joueur a choisi de quitter plutôt que retourner au menu
            if result == "quit":
                break
